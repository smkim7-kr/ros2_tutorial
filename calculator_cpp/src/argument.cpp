...
#include "rcutils/cmdline_parser.h" // parser
...
: Node("argument", node_options), // inherit Node name, option
  min_random_num_(0.0), 
  max_random_num_(0.0)
{
  ...
  const auto QOS_RKL10V =
    rclcpp::QoS(rclcpp::KeepLast(qos_depth)).reliable().durability_volatile(); // define QoS: depth=10, reliable, volatile

  arithmetic_argument_publisher_ =
    this->create_publisher<ArithmeticArgument>("arithmetic_argument", QOS_RKL10V); // create publisher <topic type>(topic name, QoS setting)

  timer_ =
    this->create_wall_timer(1s, std::bind(&Argument::publish_random_arithmetic_arguments, this)); // callback function, every 1 second
}
...
void Argument::publish_random_arithmetic_arguments() // callback function
{
  ...
  arithmetic_argument_publisher_->publish(msg); // publisher publish topic of two random floats 

  RCLCPP_INFO(this->get_logger(), "Published argument_a %.2f", msg.argument_a); // logging purpose
