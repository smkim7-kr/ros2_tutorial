...
: Node("calculator", node_options), // initialized with node name calculator
  argument_a_(0.0),
  argument_b_(0.0),
  ...
  argument_formula_("")
{
  ...
  arithmetic_argument_subscriber_ = this->create_subscription<ArithmeticArgument>(
    "arithmetic_argument",
    QOS_RKL10V,
    [this](const ArithmeticArgument::SharedPtr msg) -> void // lambda style callback function - useful for short callback function
    {
      ...
      RCLCPP_INFO(
        this->get_logger(),
        "Timestamp of the message: sec %d nanosec %d",
        msg->stamp.sec,
        msg->stamp.nanosec);
  ...
  auto get_arithmetic_operator =
    ...
    std::shared_ptr<ArithmeticOperator::Response> response) -> void // get request (operator), send response (calculated result)
    {
  ...
  // service server node w/ callback
  arithmetic_argument_server_ =
    create_service<ArithmeticOperator>("arithmetic_operator", get_arithmetic_operator);

  using namespace std::placeholders;
  // arithmetic_action_server_ : rclcpp_action::Server shared oiubter
  // message/servuce - rclcpp namespace, action - rclcpp_action namespace
...
// callback function when action client request goal
...
// callback function when action client request goal cancel
...
// callback function that execute task to acheive action goal
