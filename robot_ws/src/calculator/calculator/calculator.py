# calculator: topic subscriber (get message), 
# service server (get request, send response), 
# action server (get action goal, send feedback and result)
import time
...
class Calculator(Node):
    ...
    def __init__(self):
        ...
        self.arithmetic_action_server = ActionServer(
            ...
            ArithmeticChecker,
            'arithmetic_checker',
            self.execute_checker, # callback function
            callback_group=self.callback_group) # multithreading
    ...
    def get_arithmetic_operator(self, request, response):
        ...
        self.argument_formula = f'{self.argument_a} \
    ...
    def execute_checker(self, goal_handle): # goal_handle: action status
        self.get_loogger().info('Execute arithmetic_checker action!') # log action server started
        ...
        while total_sum < goal_sum:
            ...
            goal_handle.publish_feedback(feedback_msg) # send feedback to action client
            time.sleep(1)
        goal_handle.succeed() # send action status to action client
...
def main(args=None):
    ...
    try:
        ...
        try:
            executor.spin() # continuous spinning until callback ends
